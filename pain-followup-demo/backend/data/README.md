...
本目录是 LANShing 疼痛智能随访系统后端的数据层，负责 **两件事**：
...
另有运行时数据访问层 `database.py`（提供 `PatientDB` 等查询接口，被 Dispatcher、Episode 和审阅流程调用）。
...
| `db_config.py` | 配置 | 从 `backend/.env` 读取 `DB_HOST/DB_PORT/DB_USER/DB_PASSWORD/DB_NAME`，拼出 SQLAlchemy 连接串 `mysql+pymysql://...`。修改数据库连接信息只需改 `.env`。 |
...
| `database.py` | 运行时数据访问层 | 提供 `PatientDB` 等查询接口（被 Dispatcher、Episode 和审阅流程调用）。`_patient_to_dict` 把多表行映射成前端/随访流程使用的患者字典（含出院叙述字段）。`init_db()` 建表，并在 Patient 表为空时按配置触发基础数据播种。 |
...
- **出院叙述闭环**：`database.py._patient_to_dict` 把 `discharge_records` 的叙述字段（`discharge_summary` / `discharge_instructions` / `diagnosis_detail` / `treatment_summary`）导出到患者字典，使随访计划与 RAG 检索能够使用完整出院上下文。
