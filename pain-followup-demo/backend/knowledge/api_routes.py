...
ChromaDB 查询/写入及 embedding 计算均为同步阻塞 I/O；路由统一为同步 def，
...
def api_kb_status():
    from knowledge.store import ChromaStore
    from knowledge.embeddings import get_embedding_provider

    def _get_count():
        store = ChromaStore(get_embedding_provider())
        return store.count
    try:
        count = _get_count()
    except Exception as e:
        return {"ok": False, "error": str(e), "count": 0}
    return {"ok": True, "count": count, "collection": config.CHROMA_COLLECTION}
...
def api_kb_search(
    ...
    """语义检索知识库（embedding 向量 + ChromaDB 查询）"""
    q = q.strip()
    if not q:
