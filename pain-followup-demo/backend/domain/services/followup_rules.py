# backend/domain/services/followup_rules.py
"""
随访计划驱动的"当日随访名单"判定规则（C0~C4 纯函数）。

来源：engine/followup_scheduler.py 迁出（Agent-centric 重构 §12.2：旧名单引擎迁至
新领域层，调用点只依赖新领域接口后删除旧文件）。迁移只替换实现位置，**不修改任何
已确认的规则行为**；C0~C4 保持纯函数（仅 build_today_send_list 因计划预取按需访问
Repository 层，可注入 plan_lookup 保持纯净）。

核心判定模型 —— 一位患者"今天需要随访"在随访窗口内且无豁免时，命中任一触发即随访：
    C0 电话回访      ：连续 N 天未回复（N 由配置声明，如"连续2天没回复的患者需要电话回访"）。
                        与窗口（C2）、豁免（C4）判定一致：周期已结束/请假/复诊/住院不回访。
    C1 计划已生效    ：有一份有效随访计划（follow_up_plan 内容有效；followup_plan 表状态为 rejected 则否决）
    C2 在随访窗口内  ：discharge_date <= today <= discharge_date + duration_days（对全体患者一致）
    C3 频次命中今日  ：按 plan.frequency 解析出的节奏，today 是应随访日
    C4 无临时豁免    ：当日 daily_status 非 on_leave / revisited / hospitalized（对全体患者一致）
...
对外暴露：
    ...
    apply_today_followup_flags(pats)  → 把引擎结果回写每位患者字典的 skip_follow_up / skip_reason
    compute_next_followup(patient)    → 下次随访日（展示用）
"""
import json
import re
...
def is_followup_due(patient, today=None, plan_status=None, plan_override=None,
    ...
    no_reply_threshold：配置声明的电话回访阈值（如 2 = 连续 2 天没回复即回访）。
                       电话回访是引擎内一条判定路径（C0），与其他判定**一致**：
                       同样受随访窗口（C2）与临时豁免（C4）约束——周期已结束、
                       请假/已复诊/住院的患者，即使连续未回复也不进入回访。
    """
    ...
    no_reply_hit = (no_reply_threshold is not None
                    and consecutive_no_reply >= no_reply_threshold)

    # ---- C2 随访窗口：对全体患者一致（含电话回访）----
    if not start_date:
    ...
    # ---- C0 电话回访（阈值来自配置，窗口已在上面统一判定）----
    if no_reply_hit:
        detail["need_followup"] = True
        detail["matched_rule"] = "电话回访"
        detail["reason"] = "连续%d天未回复，需电话回访（配置阈值 %d 天）" % (
            consecutive_no_reply, no_reply_threshold)
        # 命中后仍需通过 C4 豁免（下方统一检查）

    # ---- C1/C3 计划路径 ----
    elif plan_status == "rejected":
        detail["reason"] = "随访计划未生效（已驳回）"
        return detail
    elif not plan or not frequency:
...
# 名单构建
# ==========================================================================
def build_today_send_list(patients, today=None, no_reply_threshold=None, plan_lookup=None):
    """返回当日判定名单：{total, send_count, skip_count, send_list, skip_list, details, today}。

    no_reply_threshold：电话回访阈值（配置声明，如 2），作为 C0 判定参数传入。
    plan_lookup：可选计划预取函数 ``fn(patient_id) -> plan|None``；缺省按 Repository
                 惰性读取 followup_plan 表（保持旧 build_today_send_list 行为）。
    """
    today = today or get_today()

    # 预取每位患者最新计划（用于 C1 否决 + plan_json 频次覆盖）
    if plan_lookup is None:
        def _default_plan_lookup(pid):
            from db.followup_db import get_latest_plan
            return get_latest_plan(pid)
        plan_lookup = _default_plan_lookup
    plan_map = {}
    try:
        for p in patients:
    ...
    for p in patients:
        ...
        d = is_followup_due(p, today, plan_status=plan_status, plan_override=latest,
                            no_reply_threshold=no_reply_threshold)
        details[pid] = d
        if d["need_followup"]:
            send_list.append(p)
        else:
...
def apply_today_followup_flags(patients, today=None, no_reply_threshold=None):
    """用判定规则结果回写每位患者字典的 skip_follow_up / skip_reason / followup_detail。

    方案 A 为唯一真源，方案 B 的缓存（skip_follow_up 等）由此处回写，
    保证所有读取 skip_follow_up 的既有代码得到规则驱动的结果。
    """
    res = build_today_send_list(patients, today, no_reply_threshold=no_reply_threshold)
    details = res["details"]
    for p in patients:
        pid = p.get("patient_id")
        d = details.get(pid)
...
def compute_next_followup(patient, today=None):
    """计算该患者下一次随访日（展示用）。今天命中则返回今天，否则向后找最近命中日。"""
    today = today or get_today()
